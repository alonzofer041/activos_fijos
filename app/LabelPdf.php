...
class LabelPdf extends Fpdf{
    ...
    protected $_Avery_Labels = array(
        '5158' => array('paper-size'=>'etiqueta',    'metric'=>'mm',    'marginLeft'=>20.5,    'marginTop'=>0,        'NX'=>3,    'NY'=>5,    'SpaceX'=>3.175,    'SpaceY'=>0,    'width'=>27.5,    'height'=>25.8,        'font-size'=>15),
        '5160' => array('paper-size'=>'letter',    'metric'=>'mm',    'marginLeft'=>1.762,    'marginTop'=>10.7,        'NX'=>3,    'NY'=>10,    'SpaceX'=>3.175,    'SpaceY'=>0,    'width'=>66.675,    'height'=>25.4,        'font-size'=>7),
    ...
    function __construct($format, $unit='mm', $posX=1, $posY=1) {
        ...
        // $this->SetFont('Arial');
        // $this->AddFont('Barlow','','barlow.php');       
        $this->SetMargins(0,0); 
    ...
    function Set_Font_Size($pt) {
        ...
        $this->SetFont('Barlow','B',$pt);
        $this->SetFontSize($pt);
    }

    // Print a label
    function Add_Label($text,$numinventario,$numcontrol) {
        $this->_COUNTX=0;
        if ($this->_COUNTX == $this->_X_Number) {
            ...
            if ($this->_COUNTY == $this->_Y_Number) {
                ...
                $this->_COUNTY=0;
                //$this->AddFont('barlowregular','','barlowregular.php');
               // $this->AddFont('Barlow-Bold','','Barlow-Bold.php');
                
                //$this->SetFont('barlowregular','',15);
            }
        }
        ...
        // $qrcode = new QRcode($text[0], 'H');
        // $qrcode->displayFPDF($this,$_PosX,$_PosY,20);
        // // $this->SetXY($_PosX+30, $_PosY+30);
        // $this->SetX($_PosX+20);
        // $this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height,utf8_decode('Universidad Tecnológica Metropolitana'), 0, 'L');
        // $this->ln(1);
        // $this->SetX($_PosX+20);
        // $this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height,$text[1], 0, 'L');
        $this->AddPage();

        //linea de la etiqueta de la Universidad
        $this->ln(2.5);
        $this->SetX(2.8);
        $this->SetFontSize(7.2);
        $this->SetFont('barlow','B');
        $this->SetTextColor(0,0,0);
        $this->MultiCell(49.5, 4,utf8_decode('Universidad Tecnológica Metropolitana'), 0, 'L');
        //linea de la etiqueta de la Universidad

        //linea del Qr
        $qrcode = new QRcode($text, 'H');
        $qrcode->displayFPDF($this,3,7, 17);  
        //linea del Qr

        //linea del primer identificador
        $this->ln(3.2);
        $this->SetX($_PosX);
        $this->SetFontSize(9);
        $this->SetFont('arial','B');
        $this->SetTextColor(0,0,0);
        $this->MultiCell($this->_Width - $this->_Padding, 6,utf8_decode($numinventario), 0, 'L');
           $this->SetFillColor(123,23,124);
        //linea del primer identificador

              
        //linea del segundo identificador
       $this->SetX($_PosX);
       $this->SetFontSize(9);
       $this->SetFont('arial','');
       $this->SetTextColor(0,0,0);
       $this->MultiCell($this->_Width - $this->_Padding, 6,utf8_decode(8794723), 0, 'L');
        //linea del segundo identificador
    }
